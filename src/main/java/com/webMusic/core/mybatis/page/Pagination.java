package com.webMusic.core.mybatis.page;
...
public class Pagination<T> extends SimplePage implements java.io.Serializable,
	...
	public String getSiAjaxPageHtml(){
		...
		if(this.getPageNo()>1){
			if(this.getPageNo()>5){
				pageHtml.append("<li><a href='javascript:;' onclick='"+ajaxCallBackFunctionName+"(1)'>首页</a></li>");
			}
			pageHtml.append("<li><a href='javascript:;'  onclick='"+ajaxCallBackFunctionName+"("+(this.getPageNo() - 1)+")'>上一页</a></li>");
		}
		for (int i = (this.getPageNo()-2<=0?1:this.getPageNo()-2),no = 1; i <= this.getTotalPage()&& no <6 ; i++,no++) {
			if (this.getPageNo() == i) {
				pageHtml.append("<li class='active'><a href='javascript:void(0);' >"+i+"</a></li>");
			}else{
				pageHtml.append("<li><a href='javascript:;' onclick='"+ajaxCallBackFunctionName+"("+i+")'>"+i+"</a></li>");
			}
		}
		if(this.getPageNo() < this.getTotalPage()){
			pageHtml.append("<li><a href='javascript:;'  onclick='"+ajaxCallBackFunctionName+"("+(this.getPageNo() + 1)+")'>下一页</a></li>");
		}
		pageHtml.append("</ul>");
		return pageHtml.toString();
	}
